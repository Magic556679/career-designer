...
- **App shell** (`src/main.tsx`): `StrictMode` → `QueryClientProvider` (TanStack Query, default `QueryClient`, no persistence/config yet) → `RouterProvider` (router from `src/app/router.tsx`).
- **Routing** (`src/app/router.tsx`, `src/routes/index.tsx`): data router via `createBrowserRouter`. `PublicRoutes` (in `src/routes/index.tsx`) holds the route objects (`/` → `Home`, `explore/step-1` → `Step1`) and `router.tsx` appends a `*` catch-all → `NotFound`. Add new pages to `PublicRoutes`.
- **Project structure**: feature-based. `src/routes/` holds thin route/page components (compose features, handle navigation — no domain logic). `src/features/<name>/` holds domain slices, each with its own `api/`, `components/`, `stores/`, `schema/`, and a public `index.ts` (import slices only via that barrel; slices never import each other). Shared code stays at the top level: `src/components/ui` (shadcn; `ui/spinner.tsx` also exports the app's `LoadingFallback` — `shadcn add spinner --overwrite` would wipe it), `src/components` (app-wide non-shadcn components; `errors/` holds error screens such as `errors/error-fallback.tsx`), `src/lib`, `src/stores` (global stores only — `auth`), `src/types`. Dependency direction is one-way: `routes → features → shared`.
